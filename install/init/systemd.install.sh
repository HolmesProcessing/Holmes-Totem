...
TOTEM_SCRIPT=$(sudo cat "$1"/install/init/systemd.totem.template)
TOTEM_SCRIPT=$(echo "$TOTEM_SCRIPT" | sed -e 's~INSTALL_DIRECTORY~'$2'~')
SERVICE_SCRIPT=$(sudo cat "$1"/install/init/systemd.services.template)
SERVICE_SCRIPT=$(echo "$SERVICE_SCRIPT" | sed -e 's~INSTALL_DIRECTORY~'$2'~')

echo "$TOTEM_SCRIPT" > "/etc/systemd/system/holmes-totem.service"
echo "$SERVICE_SCRIPT" > "/etc/systemd/system/holmes-totem-services.service"
