...
TOTEM_SCRIPT=$(sudo cat "$1"/install/init/upstart.totem.template)
TOTEM_SCRIPT=$(echo "$TOTEM_SCRIPT" | sed -e 's~INSTALL_DIRECTORY~'$2'~')
SERVICE_SCRIPT=$(sudo cat "$1"/install/init/upstart.services.template)
SERVICE_SCRIPT=$(echo "$SERVICE_SCRIPT" | sed -e 's~INSTALL_DIRECTORY~'$2'~')

echo "$TOTEM_SCRIPT" > "/etc/init/holmes-totem.conf"
echo "$SERVICE_SCRIPT" > "/etc/init/holmes-totem-services.conf"
